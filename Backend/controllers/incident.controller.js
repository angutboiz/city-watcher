...
const createIncident = catchAsync(async (req, res) => {
    ...
    if (!title || !desc || !category_id || !reciver_id || !location) {
        return ErrorResponse.badRequest(res, 'Vui lòng điền đầy đủ thông tin!')
    }
...
const getNearbyIncidents = async (req, res) => {
    try {
        ...
        if (lastUserLocation && lastUserLocation.longitude === parsedLongitude && lastUserLocation.latitude === parsedLatitude) {
        ...
        lastUserLocation = { longitude: parsedLongitude, latitude: parsedLatitude };

        // Lấy tất cả các sự cố từ cơ sở dữ liệu
        const incidents = await Incident.find();

        // Tính khoảng cách và lọc các sự cố gần
        const nearbyIncidents = checkNearbyIncidents(parsedLatitude, parsedLongitude, incidents);
