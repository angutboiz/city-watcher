...
export default function UserDropdown() {
    ...
    const handleLogout = async () => {
        ...
        try {
            const { data } = await axiosAPI.post("/auth/logout", { refreshToken });
            if (data.ok) {
    ...
    return (
        <div className="relative">
            <button onClick={toggleDropdown} className="flex items-center text-gray-700 dark:text-gray-400">
                ...
                <span className="block mr-1 font-medium text-theme-sm">{user?.displayName || "Chưa có đặt tên"}</span>
            ...
            <Dropdown
                ...
                <div>
                    <span className="block font-medium text-gray-700 text-theme-sm dark:text-gray-400">{user?.displayName || "Chưa có đặt tên"}</span>
